...
import {
    ...
    ScanLine, ArrowLeftRight, Move, Loader2
} from 'lucide-react';
...
const PixelScaler = () => {
    ...
    return (
        <div className="grid lg:grid-cols-12 gap-6 h-[calc(100vh-140px)] animate-fade-in">
            ...
            {/* === COLUMNA IZQUIERDA: MENÚ TABULADO === */}
                    ...
                    <>
                        <div className="flex border-b border-slate-700 bg-slate-800">
                            {[
                                ...
                                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 py-3 flex flex-col items-center gap-1 text-[10px] uppercase font-bold transition-colors border-b-2 ${activeTab === tab.id ? 'border-purple-500 text-purple-400 bg-slate-700/50' : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-700/30'}`}>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-4">
                            {activeTab === 'dims' && (
                                <div className="space-y-4 animate-in fade-in slide-in-from-left-2 duration-200">
                                    ...
                                    <div className="text-xs text-slate-500 p-2 text-center">Original: {imageSize.w}x{imageSize.h} <br /> Resultado: {finalSize.w}x{finalSize.h}</div>
                                </div>
                            )}
                            {activeTab === 'color' && (
                                <div className="space-y-4 animate-in fade-in slide-in-from-left-2 duration-200">
                                    <div className="space-y-4">
                                        ...
                                        <div><div className="flex justify-between text-xs text-slate-400 mb-1"><span>Saturación</span> <span>{saturation}%</span></div><input type="range" min="-100" max="100" value={saturation} onChange={(e) => setSaturation(Number(e.target.value))} className="w-full accent-pink-500 h-1 bg-slate-700 rounded appearance-none" /></div>
                                    </div>
                                </div>
                            )}
                            {activeTab === 'palette' && (
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {Object.entries(PRESETS).map(([key, p]) => (
                                                <button key={key} onClick={() => setPaletteMode(key)} className={`text-[10px] py-2 px-2 rounded border transition-all ${paletteMode === key ? 'bg-purple-600 border-purple-400 text-white shadow' : 'bg-slate-700 border-transparent text-slate-400 hover:bg-slate-600'}`}>{p.name}</button>
                                            ))}
                                        </div>
                                    </div>
                                    {paletteMode === 'auto' && (
                                        <div><div className="flex justify-between text-xs text-slate-400 mb-1"><span>Colores Max</span> <span>{colorCount}</span></div><input type="range" min="2" max="64" value={colorCount} onChange={(e) => setColorCount(Number(e.target.value))} className="w-full accent-pink-500 h-1 bg-slate-700 rounded appearance-none" /></div>
                                    )}
                                    <div className="bg-slate-900/50 p-3 rounded border border-slate-700">
                ...
                {!originalImage ? (
                    ...
                    <>
                        ...
                        <div className="flex justify-between items-center bg-slate-800 p-2 border-b border-slate-700 z-10 shadow-sm">
                            ...
                            {/* Indicador de carga sutil e integrado en la toolbar */}
                            <div className="flex items-center gap-2">
                                {isProcessing && (
                                    <div className="flex items-center gap-2 px-3 py-1 bg-slate-900/80 border border-purple-500/30 rounded-lg animate-in fade-in zoom-in duration-200">
                                        <Loader2 size={14} className="animate-spin text-purple-400" />
                                        <span className="text-[10px] text-purple-200 font-medium">Procesando...</span>
                                    </div>
                                )}
